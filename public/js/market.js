...
function renderizarCategorias(categorias) {
  ...
  categorias.forEach((categoria) => {
    ...
    etiqueta.className = 'category-chip';
    etiqueta.textContent = textoSeguro(categoria.nombre, 'Sin nombre');
    categoriesList.append(etiqueta);
  });
}
...
function renderizarProductos(productos) {
  ...
  productos.forEach((producto) => {
    ...
    stock.textContent = Number.isFinite(unidades)
...
async function cargarCatalogo() {
  ...
  try {
    ...
    const categorias = Array.isArray(respuestaCategorias.categorias) ? respuestaCategorias.categorias : [];
    const productos = Array.isArray(respuestaProductos.productos) ? respuestaProductos.productos : [];
